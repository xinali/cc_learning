...
// 测试静态多态
TEST(CCTest, TestCRPTStaticPolymorphism) {
...
// 测试实例计数
TEST(CCTest, TestCRPTCountInstance) {
    // 不捕获异常，但是gtest会捕获异常
    // 设置gtest不捕获异常：--gtest_catch_exceptions=0
    // lldb
    // (lldb) file out/cc_learning_test
    // (lldb) r --gtest_filter=CCTest.TestCRPTCountInstance --gtest_catch_exceptions=0
    // (lldb) bt
    ...
    //     frame #12: 0x00000001000079aa cc_learning_test`CCTest_TestCRPTCountInstance_Test::TestBody(this=0x0000600000004160) at CRPT_test.cc:83:21
